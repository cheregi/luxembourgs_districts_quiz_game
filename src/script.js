...
const citiesToShuffle = cities.slice()
...
// Count down timer
const interval = setInterval(function setTimer() {
  if (seconds > 9) {
...
const handlerEvent = function (e) {
  ...
  if (correctAnswerId === userAnswer) {
    ...
    const name = 'Name'
    const correctAnswerNameId = (correctAnswer + name)
    const goodAnswerElement = document.getElementById(correctAnswerNameId)
...
function deleteBorderColorBadAnswer() {
  const badAnswer = document.querySelector('.wrongAnswer')
  if (badAnswer != null) {
...
function win() {
  ...
  const buttonContainer = document.querySelector('h1')
...
function lose() {
  ...
  const infoNotNecessaryIfLose = document.querySelector('p')
  infoNotNecessaryIfLose.style.display = 'none'
  const moreSpaceY = document.querySelector('h1')
  const buttonPlay = document.createElement('a')
  buttonPlay.setAttribute('class', 'btn-gradient cyan mini')
  buttonPlay.innerHTML = 'Play Again'
